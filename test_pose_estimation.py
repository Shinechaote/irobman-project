from solve import segment_and_align, get_mesh_path_from_xml
...
def runner(config, num_experiments):
    ...
    sim = MjSim(config)
    for _ in tqdm(range(num_experiments), dynamic_ncols=True):
        sim.reset()
        ...
        pose, _ = segment_and_align(
            rgb, depth, intrinsic, model_path, depth_scale=1.0, visualize=False)
        ...
        pose_gt_cam = flip_matrix @ extrinsic @ pose_gt_world
...
if __name__ == "__main__":
    ...
    errors = {}
    for obj in ["Banana", "Pear", "CrackerBox", "FoamBrick", "TomatoSoupCan", "MustardBottle"]:
        ...
        errors[obj] = runner(config, 10)

    for key in errors:
        print(key, np.mean(errors[key]), np.std(errors[key]))
